...
from dao.desktop import _setup_key, main, workspace_path
from dao.state import Store
from dao.windows_credentials import _open_clipboard, load_openai_key, save_openai_key
...
class DesktopTests(unittest.TestCase):
    def test_no_arguments_creates_per_user_workspace_then_opens_chat(self):
        with tempfile.TemporaryDirectory() as temp:
            with patch.dict(os.environ, {"LOCALAPPDATA": temp, "OPENAI_API_KEY": "fake"}):
                with patch("dao.desktop.cli_main", return_value=0) as cli:
                    self.assertEqual(main([]), 0)
                    self.assertEqual(main([]), 0)
                workspace = workspace_path()
                self.assertEqual(workspace, Path(temp) / "Dao" / "workspace")
    ...
    def test_arguments_retain_regular_cli(self):
        with patch("dao.desktop.cli_main", return_value=0) as cli, \
             patch("dao.desktop.load_openai_key") as load:
    ...
    def test_explicit_cli_uses_saved_key(self):
        with patch.dict(os.environ, {}, clear=True), \
             patch("dao.desktop.load_openai_key", return_value="saved-key"):
            def run_cli(args):
                self.assertEqual(os.environ.get("OPENAI_API_KEY"), "saved-key")
                return 0

            with patch("dao.desktop.cli_main", side_effect=run_cli) as cli:
    ...
    def test_local_cli_command_does_not_load_saved_key(self):
        with patch.dict(os.environ, {}, clear=True), \
             patch("dao.desktop.load_openai_key") as load, \
             patch("dao.desktop.cli_main", return_value=0) as cli:
    ...
    def test_explicit_ask_loads_saved_key(self):
        with patch.dict(os.environ, {}, clear=True), \
             patch("dao.desktop.load_openai_key", return_value="saved-key") as load, \
             patch("dao.desktop.cli_main", return_value=0) as cli:
    ...
    def test_clipboard_entry_saves_only_when_chosen(self):
        with patch.dict(os.environ, {}, clear=True):
            with patch("dao.desktop.load_openai_key", return_value=None), \
                 patch("dao.desktop.read_clipboard_text", return_value="  fake-key  "), \
                 patch("builtins.input", side_effect=["", "y"]), \
                 patch("dao.desktop.save_openai_key") as save:
                _setup_key()
    ...
    def test_direct_paste_at_first_prompt_preserves_key_case(self):
        ...
        output = StringIO()
        with patch.dict(os.environ, {}, clear=True), \
             patch("dao.desktop.load_openai_key", return_value=None), \
    ...
    def test_empty_clipboard_can_be_retried_or_skipped(self):
        with patch.dict(os.environ, {}, clear=True):
            with patch("dao.desktop.load_openai_key", return_value=None), \
                 patch("dao.desktop.read_clipboard_text", return_value=None), \
    ...
    def test_saved_key_skips_prompt(self):
        with patch.dict(os.environ, {}, clear=True):
            with patch("dao.desktop.load_openai_key", return_value="saved-key"), \
                 patch("dao.desktop.getpass.getpass") as prompt:
                _setup_key()
                self.assertEqual(os.environ["OPENAI_API_KEY"], "saved-key")
    ...
    def test_clipboard_open_retries_after_transient_lock(self):
        ...
        clipboard = Clipboard()
        with patch("dao.windows_credentials.time.sleep") as sleep:
    ...
    def test_workspace_initialization_error_is_reported(self):
        error = StringIO()
        with patch("dao.desktop.Store", side_effect=OSError("disk unavailable")), \
             patch("dao.desktop.cli_main") as cli, redirect_stderr(error):
